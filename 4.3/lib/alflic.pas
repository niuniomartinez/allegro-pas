UNIT alflic;
(*< There are two high level functions for playing FLI/FLC animations: @link
    (al_play_fli), which reads the data directly from disk, and  @link
    (play_memory_fli), which uses data that has already been loaded into RAM.
    Apart from the different sources of the data, these two functions behave
    identically.  They draw the animation onto the specified bitmap, which
...
(* Plays an Autodesk Animator FLI or FLC animation file on the specified @link
   (AL_BITMAP), reading the data from disk as it is required.  If @code (loop)
   is @true, the player will cycle when it reaches the end of the file,
   otherwise it will play through the animation once and then return.  Read the
   introduction for @code (alflic) unit for a description of the @code
   (callback) parameter.
   @returns (@code (AL_FLI_OK) if it reached the end of the file, @code
    (AL_FLI_ERROR) if something went wrong, or the value returned by the @code
...
(* Plays an Autodesk Animator FLI or FLC animation on the specified @link
   (AL_BITMAP), reading the data from a copy of the file which is held in
...
(* These variables are set by @link (al_next_fli_frame) to indicate which part
   of the @link (al_fli_bitmap) has changed since the last call to @link
   (al_reset_fli_variables).  If @code (fli_bmp_dirty_from) is greater than
...
(* These variables are set by @link (al_next_fli_frame) to indicate which part
   of the @link (al_fli_palette) has changed since the last call to @link
   (reset_fli_variables).  If @code (al_fli_pal_dirty_from) is greater than
   @code (al_fli_pal_dirty_to), the palette has not changed, otherwise colors
   @code (al_fli_pal_dirty_from) to @code (al_fli_pal_dirty_to) (inclusive)
   have altered.  You can use these when updating the hardware palette, to
   avoid unnecessary calls to @link (al_set_palette). *)
...
(* Reads the next frame of the current animation file.  If @code (loop) is
   ...
   @returns @code (AL_FLI_OK) on success, @code (AL_FLI_ERROR) or @code
   (AL_FLI_NOT_OPEN) on error, and @code (AL_FLI_EOF) on reaching the end of
   the file. *)
  FUNCTION al_next_fli_frame (loop: BOOLEAN): LONGINT;

(* Once you have done whatever you are going to do with the @link
   (al_fli_bitmap) and @link (al_fli_palette), call this function to reset the
   @code (al_fli_bmp_dirty_* )and @link (al_fli_pal_dirty_* )variables. *)
