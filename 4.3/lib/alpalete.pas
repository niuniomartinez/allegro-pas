...
(* To speed up reducing RGB values to 8-bit paletted colors, Allegro uses a 32k
   ...
   also vastly accelerate the @link (al_makecol) and some @code
   (al_create_*_table) functions on 8-bit graphic mode.  RGB tables can be
...
(* Sets the entire palette of 256 colors.  You should provide an array of 256
   RGB structures.  Unlike @link (al_set_color), there is no need to call @link
   (vsync) before this function. *)
  PROCEDURE al_set_palette (p: AL_PALETTE); CDECL;
    EXTERNAL ALLEGRO_SHARED_LIBRARY_NAME NAME 'set_palette';

(* Sets the palette entries between @code (from) and @link (ato) (inclusive:
...
(* Retrieves the entire palette of 256 colors.  You should provide a @link
   (AL_PALETTE) to store it in. *)
  PROCEDURE al_get_palette (p: AL_PALETTEptr); CDECL;
    EXTERNAL ALLEGRO_SHARED_LIBRARY_NAME NAME 'get_palette';

(* Retrieves the palette entries between @code (from) and @link (ato)
...
(* Calculates a temporary palette part way between @code (source) and @code
   (dest), returning it in the @code (aoutput) parameter.  The position between
...
(* Gradually fades from a black screen to the specified palette.  The @code
...
(* Gradually fades from the current palette to a black screen.  The @code
   (speed) is from 1 (the slowest) up to 64 (instantaneous).
...
(* Restores the palette tables that were in use before the last call to @link
   (al_select_palette). *)
