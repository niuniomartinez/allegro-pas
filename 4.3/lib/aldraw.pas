...
(* Each bitmap has an associated clipping rectangle, which is the area of the
   ...
   likely to be altered (and so the coordinates returned by @link
   (al_get_clip_rect) will be different).  However, such modifications are
...
@unorderedList (
  ...
  @item (@link (AL_RAW_MODE_SOLID_PATTERN))
  @item (@link (AL_RAW_MODE_MASKED_PATTERN))
  @item (@link (AL_RAW_MODE_TRANS))
)
   ...
   one is not.  The pattern is tiled in a grid starting at point @code
...
(* This is a shortcut for toggling xor drawing mode on and off.  Calling @code
   (al_xor_mode @(1@)) is equivalent to @link (al_drawing_mode) @code
   (@(AL_DRAW_MODE_XOR, NIL, 0, 0@)).  Calling @code (al_xor_mode @(0@)) is
