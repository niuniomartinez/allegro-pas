...
(* Triggers a sample at the specified volume, pan position, and frequency.  The
   ...
   @code (loop) is not zero, the sample will repeat until you call @link
   (al_stop_sample), and can be manipulated while it is playing by calling
...
(* Stop a sample from playing, which is required if you have set a sample going
   ...
   stop them all. You must still destroy the sample using @link
   (al_destroy_sample). *)
