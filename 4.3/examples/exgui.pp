...
(* for the d_edit_proc object *)
  LEN = 32;
VAR
  TheString: STRING[255] = 'Change Me!';

(* for the d_text_box_proc object *)
  TheText: STRING =
   'I''m text inside a text box.'+Chr(13)+Chr(13)+
   'I can have multiple lines.'+Chr(13)+Chr(13)+
...
BEGIN
  al_alert ('Selected menu item:', '', al_active_menu^.text, 'Ok', '', 0, 0);
...
BEGIN
  ...
  IF (al_active_menu^.flags AND AL_D_SELECTED) <> 0 THEN
    al_active_menu^.text := 'Checked'
  ELSE
    al_active_menu^.text := 'Unchecked';
...
(* the submenu *)
  Submenu: ARRAY [0..4] OF AL_MENU = (
    ( 'Submenu',            NIL,   NIL, AL_D_DISABLED,    NIL  ),
    ( '',                   NIL,   NIL, 0,                NIL  ),
    ( 'Checked', @CheckCallback,   NIL, AL_D_SELECTED,    NIL  ),
    ( 'Disabled',           NIL,   NIL, AL_D_DISABLED,    NIL  ),
    ( NIL,                  NIL,   NIL, 0,                NIL  )
  );
