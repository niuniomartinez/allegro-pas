...
{ Key pressed. }
  ...
  { Since the game will run in 320x240 pixel screen, we need a buffer to make
    ...
    { Use the keyboard to move arround the map. }
      IF al_keyboard_needs_poll <> 0 THEN al_poll_keyboard;
  ...
  PROCEDURE ActionKeys (VAR Key: AL_INT);
  BEGIN
  { Deactivate the close button. }
    al_set_close_button_callback (NIL);
  ...
  PROCEDURE CursorKeys (Key: AL_INT; VAR cx, cy: INTEGER);
  BEGIN
...
(* Main loop. *)
  ...
  { Fair play.  Releases some CPU pressure. }
    al_rest (1);
  { Mouse. }
    IF al_mouse_needs_poll <> 0 THEN al_poll_mouse;
